...
class DokumenController extends Controller
{
    ...
    //     $tps = Tps::with('desa.kecamatan')->findOrFail($user->tps_id);

    //     // Ambil dokumen yang sudah ada per jenis
    //     $uploaded = Dokumen::where('tps_id', $tps->id)
    //         ->get()
    //         ->keyBy('jenis'); // ['PPWP' => Dokumen, ...]

    //     return view('dokumen.upload', compact('tps', 'uploaded'));
    // }
    ...
    //     $tps = Tps::findOrFail($user->tps_id);
    ...
    //     // Kalau admin sedang view-as-pps, pakai desa dari session
    ...
    private function authorizeAccess(Dokumen $dokumen): void
    {
        ...
        $allowed = match($user->role) {
            'admin' => true,
            'ppk'   => Desa::where('kecamatan_id', $user->kecamatan_id)
