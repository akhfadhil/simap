...
{{-- Filter --}}
<form method="GET" class="flex gap-3 mb-6 flex-wrap" id="filter-form">
    ...
    @endif
    <span class="font-mono2 text-[10px] text-gray-600 self-center">{{ $tpsList->count() }} TPS</span>
</form>
...
<div class="bg-[#141414] border border-gray-800 mb-4">
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-800 cursor-pointer"
        ...
        <div class="flex items-center gap-3">
            <div class="w-24 h-1 bg-gray-800 rounded-full">
                <div class="h-1 rounded-full bg-brand" style="width:{{ ($tps->dokumens->count()/5)*100 }}%"></div>
            </div>
    ...
    <div class="flex items-center justify-between px-6 py-3.5 border-b border-gray-900 last:border-0 flex-wrap gap-2">
        <div class="flex items-center gap-3">
            <div class="w-1.5 h-1.5 rounded-full" style="background: {{ $dok ? ($dok->status==='terverifikasi'?'#2EC4B6':'#F4A261') : '#333' }}"></div>
            <div>
        ...
        <div class="flex items-center gap-2">
            <span class="font-mono2 text-[9px] tracking-widest uppercase px-2 py-1"
                  style="color:{{ App\Models\Dokumen::STATUS_COLORS[$dok->status] }};background:{{ App\Models\Dokumen::STATUS_COLORS[$dok->status] }}15;border:1px solid {{ App\Models\Dokumen::STATUS_COLORS[$dok->status] }}33">
                {{ App\Models\Dokumen::STATUS_LABELS[$dok->status] }}
            </span>
            <a href="{{ route('dokumen.preview', $dok) }}" target="_blank"
               class="border border-gray-700 text-gray-500 px-3 py-1 font-mono2 text-[10px] uppercase tracking-widest hover:border-gray-500 hover:text-gray-300 transition">PREVIEW</a>
            <a href="{{ route('dokumen.download', $dok) }}"
               class="border border-gray-700 text-gray-500 px-3 py-1 font-mono2 text-[10px] uppercase tracking-widest hover:border-gray-500 hover:text-gray-300 transition">UNDUH</a>
        </div>
        @else
